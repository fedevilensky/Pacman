...
public class StrategyAmbushTest : StrategySelector
{
    private float chaseTimer = 0f;
    private Behaviour chase = new BehaviourChase();
    private Behaviour ambush = new BehaviourAmbush();
    private float MaxWanderTime = 13f;
    private float MaxChaseTime = 5f;
    private float distance;
    private float minDistance = 10f;

    public StrategyAmbushTest()
    {
        ChangeBehaviour(ambush);
    }
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (chaseTimer >= MaxWanderTime && activeBehaviour == ambush)
        {
